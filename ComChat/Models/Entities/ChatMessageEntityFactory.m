...
+ (id)entityWithDictionary:(NSDictionary *)dic
{
    if (dic) {
        ...
        //将图片直接缓存到本地只留一个路径在entity里面保存
        NSData * imageData = [[NSData alloc]initWithBase64EncodedString:dic[@"imageStr"] options:0];
        NSString *path_sandox = NSHomeDirectory();
        //设置一个图片的存储路径
        NSString *imagePath = [path_sandox stringByAppendingString:[NSString stringWithFormat:@"/Documents/%@",dic[@"imageName"]]];
...
+ (id)entityWithDictionary:(NSDictionary *)dict
{
    if (dict && [dict isKindOfClass:[NSDictionary class]]) {
        ...
        NSString * fileWavPath = [cacheDir stringByAppendingPathComponent:dict[@"voiceName"]];
        
        //如果这个文件存在，那么久不用缓存
        NSFileManager * fileManager = [NSFileManager defaultManager];
        if (![fileManager fileExistsAtPath:fileWavPath]) {
            NSData * data = [[NSData alloc]initWithBase64EncodedString:dict[@"voiceStr"] options:0];
            [data writeToFile:fileWavPath atomically:YES];
        }
        
        entity.time = [dict[@"time"] integerValue];
        entity.url = fileWavPath;
        return entity;
    }
    return nil;
}
...
- (void)playAudioWithProgressBlock:(void (^)(CGFloat))progressBlock
{
    ...
    NSString * baseData = [data base64EncodedStringWithOptions:0];
    
    if (self.isOutgoing) {
//        [[AudioRecordPlayManager sharedManager] playWithUrl:self.url];
        [[AudioRecordPlayManager sharedManager] playWithData:baseData];
        progressBlock(1.0f);
    }
    else {
//        [[AudioRecordPlayManager sharedManager] playWithUrl:self.url];
