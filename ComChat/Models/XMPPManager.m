...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
    ...
    if ([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive)
    {
        ...
        //TODO: 李小涛//如果程序在后台运行，收到消息以通知类型来显示
...
//
//            id dic = [ChatMessageEntityFactory objectFromJSONString:message.body];
//            if ([dic isKindOfClass:[NSDictionary class]]) {
//                userInfo = dic;
//                type = [userInfo objectForKey:@"type"];
//                content = [[NSString alloc]init];
//                if ([type isEqualToString:@"text"]) {
//                    content = [userInfo objectForKey:@"data"];
...
//            }
//            //读取未读数据
//            NSNumber * iconCount = 0;
//            NSUserDefaults * userDefault = [NSUserDefaults standardUserDefaults];
//            if(!userDefault){
//                iconCount = [NSNumber numberWithInt:0];
//            }else{
//                iconCount = [userDefault objectForKey:ICON_COUNT];
//            }
//            iconCount = [NSNumber numberWithInt:(iconCount.intValue + 1)];
//            [userDefault setObject:iconCount forKey:ICON_COUNT];
//            [userDefault synchronize];
//            
//            
//            UILocalNotification *localNotification = [[UILocalNotification alloc] init];
//            localNotification.alertBody = [NSString stringWithFormat:@"%@:%@",message.from.user,content];//通知主体
//            
//            localNotification.soundName = UILocalNotificationDefaultSoundName;//通知声音
//            localNotification.applicationIconBadgeNumber = iconCount.integerValue;//标记数
