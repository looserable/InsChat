...
//TODO: 李小涛：这里是通过一个unread的字段而不是通过删除一条数据来处理的。找到那个联系人对应的对话，然后更新它的unRead字段为0;
#pragma mark 重置未读消息数
...
- (void)decreaseTotalUnreadMessagesCountWithValue:(NSInteger)count
{
    ...
    NSNumber * iconCount = self.totalUnreadMessagesNum;
    //TODO: 李小涛添加它只是为了得到正确的未读角标，但存在一个问题，那就是其他的数据的未读的消息来源同样需要处理
...
//TODO: XMPPMessageArchiving_Contact_CoreDataObject 最近联系人的coredata对象
...
//通过聊天页面的cell的刷新去请求最近的联系人以及最近聊天的群组。
