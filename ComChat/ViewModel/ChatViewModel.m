...
//    NSString *JSONString = [ChatMessageImageEntity JSONStringWithImageWidth:newImage.size.width height:newImage.size.height url:url];
    ...
    //这里将data的数据经过base64加工后产生一个字符串，加载xml的body里面
    NSString * imageStr = [imageData base64EncodedStringWithOptions:0];
    
    NSString * JSONString = [ChatMessageImageEntity JSONStringWithImageWidth:newImage.size.width height:newImage.size.height imageName:imageName imageStr:imageStr];
...
{
    NSLog(@"音频发送中...");
    NSString *base64Data = [voiceData base64EncodedStringWithOptions:0];
    NSString *JSONString = [ChatMessageVoiceEntity JSONStringWithAudioTime:time voiceStr:base64Data name:voiceName];
    [[XMPPManager sharedManager] sendChatMessage:JSONString toJID:self.buddyJID];
...
- (XMPPMessageArchiving_Message_CoreDataObject *)objectAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"number of objects = %ld,indexpath = %ld",[sectionInfo numberOfObjects],indexPath.row);
